              ...
              <div class="alert alert-success alert-dismissable">
                <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
                  <?php echo $this->session->flashdata('success_message'); ?> </div>
                    <?php } ?>
                  <div class="x_content">
                    <form enctype="multipart/form-data" method="post" class="form-horizontal" id="add_category" name="add_category" data-toggle="validator" action="<?php echo site_url("category/insert_sub_category"); ?>">
                      <div class="col-md-8 col-xs-12">
                           <div class="item form-group">
                             <label class="control-label1 col-xs-12" for="product">Parent category</label><br>
                              <div class="col-xs-12">
                                <select class="cat_id form-control" name="parent_category" id="parent_category">
                                  ...
                                  <?php foreach ($parent_cat_list as $key => $cat_list) {?>
                                            <option value=<?=$cat_list->category_id?>><?=$cat_list->cat_name?></option>
                                  <?php }?>
                                </select>
                              </div>
                            </div>
                            <div class="item form-group">
                             <label class="control-label1 col-xs-12" for="product">Sub Category</label><br>
                              <div class="col-xs-12">
                                <input id="category_name" class="form-control col-md-7 col-xs-12" name="category_name" type="text">
                              </div>
                            </div>
                            <!-- <div class="item form-group">
                              <label class="control-label1 col-xs-12" for="product">Sub category</label><br>
                        ...
                        <div class="form-group">
                          <div class="col-md-6 col-md-offset-3" id="eventsExample">
                            <input type="button" onclick="location.href = '<?php echo base_url(); ?>category/subcategory'" class="btn btn-primary" value="Cancel">
                            <button id="send" type="submit" class="btn btn-success btn_add">Add</button>
                          </div>
                        </div>
                      </form>
                    </div>
...
$(document).on('click', ".btn_add", function () {
  ...
  if(parent_category == ''){
    swal({
        ...
        return false;
        $('html, body').animate({
                scrollTop: $('#parent_category').offset().top
            }, 'slow');
      })
      return false;
  }

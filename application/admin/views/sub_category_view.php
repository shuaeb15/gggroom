    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content">
            <div class="res"></div>
              <div class="col-md-12"><a class="btn btn-success col-md-3 pull-right" href="<?php echo site_url('category/add_sub_category') ?>">Add sub category</a></div>
              ...
              <tbody>
                  <?php foreach ($category_list as $key => $value) { ?>
                    <?php if($value->child_category->cat_name == '-'){ ?>
                    ...
                    <!-- <td><?=$value->child_category->cat_name?></td> -->
                    <td>
                      ...
                      <button type="button" name="button" class="cls-btn-active-class btn cls_delete_btn"  data-user-id="<?=$value->category_id?>" style="padding:5px 10px 5px 10px !important;width:70px;background: #40E0D0;border-radius:3px;border:none;color:white;" onclick="SubcategoryConfirmDelete('<?=$value->category_id?>')" id="<?=$value->category_id?>">Delete</button>
                      ...
                      <!-- <a href="<?=site_url()?>category/edit_sub_category/<?=$value->category_id?>"><i class="fa fa-pencil"></i></a> |
                      <a href="javascript:void(0)" onclick="SubcategoryConfirmDelete('<?=$value->category_id?>')" data-original-title="View" id="<?=$value->category_id?>" data-toggle="tooltip"><i class="fa fa-remove"></i></a> -->
                    </td>
                    </tr>
                  <?php } ?>
                  <?php }?>
...
$(document).on('click', ".cls_edit_btn", function (e) {
  ...
  window.location.href = site_url + "category/edit_sub_category/" + cat_id;
});

    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content">
            <div class="res"></div>
            <div class="col-md-12"><a class="btn btn-success col-md-3 pull-right" href="<?php echo site_url('location/add_state') ?>">Add State</a></div>
              <table id="item-list" class="table table-striped table-bordered">
                <thead>
                  <tr>
                    ...
                    <th>Breaking start time</th>
                    <th>Breaking end time</th>
                    <th>Date</th>
                    <th>Action</th>
                 </tr>
               </thead>
              <tbody>
                <?php foreach ($state_list as $key => $value) {
                  ...
                  if(!empty($value->break_start_time)){
                    if($value->break_start_time == '00:00:00'){
                      $break_start_time = '';
                    }else{
                      $break_start_time = date("g:i A", strtotime($value->break_start_time));
                    }
                  }else{
                      $break_start_time = '';
                  }

                  if(!empty($value->break_end_time)){
                    if($value->break_end_time == '00:00:00'){
                      $break_end_time = '';
                    }else{
                      $break_end_time = date("g:i A", strtotime($value->break_end_time));
                    }
                  }else{
                      $break_end_time = '';
                  }?>
                  ...
                  <td><?=$start_time?></td>
                  <td><?=$end_time?></td>
                  <td><?=$break_start_time?></td>
                  <td><?=$break_end_time?></td>
                  <td><?=$date?></td>
                  <td>
...
$(document).on('click', ".cls_edit_btn", function (e) {
  ...
  window.location.href = site_url + "location/edit_state/" + service_id;
});
    ...
    $(document).on('click', ".btn_main", function (e) {
      ...
      if ($(this).hasClass("price-filter-active")) {
        ...
        function(isConfirm) {
          if (isConfirm) {
            $.ajax({
                url: site_url + 'location/active_state_id',
                method: "POST",
                data: {id: page_id},
                async: false,
                success: function (data) {
                    ...
                    if (obj.success == 'success') {
                      ...
                      $(".btn_active"+ obj.id).html('Active');
                        $('.res').html('<div class="alert alert-success alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>State active successfully!</div>');
                    }
        ...
        function(isConfirm) {
          if (isConfirm) {
            $.ajax({
                url: site_url + 'location/inactive_state_id',
                method: "POST",
                data: {id: page_id},
                async: false,
                success: function (data) {
                    ...
                    if (obj.success == 'success') {
                      ...
                      $(".btn_active"+ obj.id).html('Inactive');
                        $('#row_' + obj.id).remove();
                        $('.res').html('<div class="alert alert-success alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>State was set to inactive.</div>');
                    }
                    else if (obj.unsuccess == 'unsuccess') {
                        $('.res').html('<div class="alert alert-danger alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>unsuccessfully!</div>');

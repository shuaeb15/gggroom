            ...
            <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
              ...
              <div class="alert alert-success alert-dismissable">
                <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
                  ...
                  <div class="x_content">
                    <form enctype="multipart/form-data" method="post" class="form-horizontal" id="edit_offer_form" name="edit_offer_form" data-toggle="validator" action="<?php echo site_url("offers/update_offer"); ?>">
                      <input type="hidden" name="offer_id" id="offer_id" value="<?=$offer_data->id?>"/>
                      <div class="col-md-8 col-xs-12">
                         <div class="item form-group">
                           <label class="control-label1 col-xs-12" for="product">Offer code</label><br>
                            <div class="col-xs-12">
                              <input id="offer_code" class="form-control col-md-7 col-xs-12" name="offer_code" type="text" value="<?php if(!empty($offer_data)){ echo $offer_data->code;}?>">
                            </div>
                          </div>
                        </div>
                        <div class="col-md-8 col-xs-12">
                             ...
                             <label class="control-label1 col-xs-12" for="product">Discount</label><br>
                              <div class="col-xs-12">
                                <select class="form-control col-md-7 col-xs-12" name="discount_type" id="discount_type">
                                  <option <?php if(!empty($offer_data)){ if($offer_data->discount_type == ''){ echo 'selected';}}?> value="">select</option>
                                  <option <?php if(!empty($offer_data)){ if($offer_data->discount_type == '1'){ echo 'selected';}}?> value="1">Percent discount</option>
                                  <option <?php if(!empty($offer_data)){ if($offer_data->discount_type == '2'){ echo 'selected';}}?> value="2">Fixed amount</option>
                                </select>
                              </div>
                              <div class="col-xs-12">
                                <input id="price" class="form-control col-md-7 col-xs-12" name="price" type="number" style="margin-top:10px;" placeholder="Enter price" value="<?php if(!empty($offer_data)){ echo $offer_data->price;}?>">
                              </div>
                            </div>
                        </div>
                        <div class="col-md-8 col-xs-12">
                          ...
                          <div class="item form-group">
                             <div class="col-xs-12 cls-chk" style="font-size: 19px !important;">
                                 <input type="radio" class="offer_radio_type" name="offer_radio_type" id="offer_radio_type" value="1" <?php if(!empty($offer_data)){ if($offer_data->offer_type == '1'){ echo 'checked';}}?>>General Offer
                                 <input type="radio" class="cls-chk-input offer_radio_type" name="offer_radio_type" id="offer_radio_type" value="2" <?php if(!empty($offer_data)){ if($offer_data->offer_type == '2'){ echo 'checked';}}?>>Service Offer
                             </div>
                           </div>
                        </div>
                        <div class="col-md-8 col-xs-12 cls_service">
                          <div class="item form-group">
                            ...
                            <div class="col-xs-12">
                              <input type="hidden" class="form-control col-md-7 col-xs-12" name="service_id" id="service_id" value="<?php if(!empty($offer_data)){ echo $offer_data->service_id;}?>">
                              ...
                              <select class="form-control col-md-7 col-xs-12" name="service_name" id="service_name">
                                ...
                                <?php foreach ($services_list as $key => $value): ?>
                                  <option <?php if(!empty($offer_data)){ if($offer_data->service_id == $value->id){ echo 'selected';}}?> value="<?=$value->id?>"><?=$value->service_name?> - <?=$value->shop_name?></option>
                                <?php endforeach; ?>
                              </select>
                            </div>
                          </div>

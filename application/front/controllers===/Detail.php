...
class Detail extends MY_Controller {
    ...
    public function shop_detail($service_id) {
      ...
      $service_list->shop_id = $this->uri->segment(4);
        $ashop_id = $this->uri->segment(4);
      $footer_pages1 = $this->general_model->get_footer_page_data('*', 'page', array('flag' => 1, 'is_deleted' => 0));
      $this->data['footer_pages1'] = $footer_pages1;
      ...
      $service_list = $this->general_model->get_service_data_id_shop_name('services', array('services.id' => $service_id, 'services.is_deleted' => 0),$service_list->shop_id);
      ...
      $main_filter_shop_list = $this->general_model->get_related_services_new('services', array('services.id !=' => $service_id, 'services.cat_id' => $service_list->cat_id, 'services.is_deleted' => 0, 'user.u_category' => 2),$ashop_id);
      ...
      foreach ($main_filter_shop_list as $key => $value) {
        ...
        $main_services = $this->general_model->get_service_data_by_worker_shopid_new1( 'services', array('services.id' => $value->id,  'services.is_deleted' => 0), $ashop_id );
      ...
      $my_shop = $this->db->select('service_id')->from('shop')->where('id', $ashop_id)->get()->result_array();
      $my_shop_id = $my_shop[0]['service_id'];
      $main_filter_shop_list1 = $this->db->select('services.*, shop.shop_name, shop.id as shop_id, category.parent_id')->from('services')->where('services.is_deleted','0')->where("FIND_IN_SET(services.id,'$my_shop_id') !=", 0)->join('shop', 'shop.id='.$ashop_id, 'left')->join('category', 'services.cat_id=category.category_id', 'left')->get()->result();


     // $main_filter_shop_list1 = $this->general_model->get_other_services_new('services', array('services.id !=' => $service_id,  'services.is_deleted' => 0),$ashop_id);

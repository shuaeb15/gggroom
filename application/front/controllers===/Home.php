...
class Home extends MY_Controller {
    ...
    public function filter_services(){
      ...
      if($filter_shops != ''){
      $my_shop = $this->db->select('service_id')->from('shop')->where('id',  $filter_shops)->get()->result_array();
      $filter_shops1 = $my_shop[0]['service_id'];
      //$main_filter_shop_list = $this->db->select('services.*')->from('services')->where('services.is_deleted','0')->where("FIND_IN_SET(services.id,'$my_shop_id') !=", 0)->get()->result();
      }
      else
      {
          $filter_shops1 = '';

...
class Appointment extends MY_Controller {
    ...
    public function get_worker_data(){
      ...
      $worker_list = $this->general_model->get_worker_data_id_appointment('services', array('services.is_deleted' => 0),$service_id);
        ...
        // $worker_list1 = $this->db->select('*')->from('workers')
        // ->where('is_deleted','0')
        // ->where("FIND_IN_SET($service_id,`service_id`)!=",0)
        // ->where("FIND_IN_SET($shop_id,`shop_id`)!=",0)
        // ->get()->result();
        // print_r($worker_list1);
       
 // echo '<pre>',print_r($worker_list), '<pre>'; exit();
      $worker_data = [];
      $i = 0;
      foreach ($worker_list as $key => $value) {
        ...
        //   foreach ($w_id as $key1 => $worker) {
            ...
            //  $cat_list11 = $this->general_model->get_workers_category1( 'workers', array('workers.id' => $worker,'workers.shop_id' => $shop_id, 'workers.is_deleted' => 0));

              // $cat_list11 = $this->general_model->get_workers_category1( 'workers', array('workers.id' => $worker,  FIND_IN_SET("2", shop_id) ,  'workers.is_deleted' => 0));
    //          $cat_list1 = $this->db->select('workers.id as worker_id, workers.name as worker_name')->from('workers')->where('id',$worker)->where('is_deleted','0')
    // ->where("FIND_IN_SET($shop_id,shop_id)")->get()->result();

    //  $cat_list1 = $this->db->select('workers.id as worker_id, workers.name as worker_name')->from('workers')->where('is_deleted','0')
    // ->where("FIND_IN_SET($shop_id,shop_id)")->where("FIND_IN_SET($value->id,service_id)")->get()->result();

     $cat_list1 = $this->db->select('workers.id as worker_id, workers.name as worker_name')->from('workers')->where('is_deleted','0')->where("FIND_IN_SET('$shop_id',shop_id) !=", 0)->get()->result();

    // echo $shop_id;
     //echo '<pre>';print_r($cat_list1), '<pre>';
   ...
   // print_r($cat_list11);
            ...
            foreach ($cat_list1 as $key => $value) {
             ...
             // if(!empty($cat_list1)){
                $worker_data[$i][$j] = array('worker_id'=>$value->worker_id,'worker_name'=>$value->worker_name);

                $all_review_list = $this->general_model->count_rating_review( 'rating_review', array('worker_id' =>$value->worker_id, 'shop_id' => $shop_id, 'service_id' => $service_id[$key], 'is_deleted' => 0));

        ...
        if($categories && $categories != ''){
          ...
          foreach($categories as $k=>$v){
            ...
            $cat_data = array(
              ...
              'worker_id' => $workers,
              // 'id' => $v->id,
              'type' => $_POST['worker_permission'],
              'cat_id' => $v->cat_id,
              'service_name' => $v->serviceName,
              'price' => $v->price,
              'time' => $v->duration,
              'shop_id' => $_POST['radiog_list'],
              'image' => $image,
              'created_at' => date('Y-m-d H:i:s'),
              'is_active' => 1,
              'is_deleted' => 0
            );
            $checkData = array('id'=>$v->id, 'cat_id' => $v->cat_id);
            // echo "<pre>";   print_r($checkData); exit;
            $service_exists = $this->general_model->check_exist_data("*", 'services', $checkData);
            // echo "<pre>";   print_r($service_exists); exit;
            if($service_exists == ''){
              // $insert_id = $this->general_model->insert_services_data($cat_data, 'services');
              // $insert_id = $this->general_model->insert_all_services($data, 'services');
              // if($insert_id){
              //   $this->session->set_flashdata('success_message', "Service added successfully");
              //   // redirect('service', 'refresh');
